...
- Benchmark corpus unchanged by the original Phase A/B build, at 138 scenarios, precision/recall 1.000/1.000 (see `redflag-ci-adversarial-tests/benchmark/RESULTS.md`) -- deliberately: the corpus format is one before/after file pair per scenario ID representing a single PR, and genuinely simulating a merge-then-PR sequence would mean building baseline/Octokit simulation into the benchmark runner itself. Cross-PR behavior is covered instead by integration tests exercising the real webhook-to-comment pipeline against a mocked baseline. See `redflag-ci-adversarial-tests/STRESS_TESTING.md`.
