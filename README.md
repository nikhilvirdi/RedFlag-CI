...
**Latest release: [v2.0.0](https://github.com/nikhilvirdi/RedFlag-CI/releases/tag/v2.0.0)** -- the project's final planned version. See the release page for the full writeup, or `redflag-ci-adversarial-tests/TRANSPARENCY_REPORT.md` in this repo for the post-ship audit behind it.
...
- **Benchmarked, not just claimed.** A 139-scenario adversarial test corpus backs the precision and recall numbers below, with every known limitation documented openly rather than hidden. See `redflag-ci-adversarial-tests/STRESS_TESTING.md`.
...
v2.0.0 is complete -- the project's final planned release, since hardened by a full post-ship audit (Stage 3; see `redflag-ci-adversarial-tests/TRANSPARENCY_REPORT.md`). The core pipeline (thirteen detector IDs total: the original six, six new ones added in v1.2.0 -- RF-1/RF-2's JSON-key extension reuses existing IDs rather than adding new ones, see `architecture.md` §5 -- plus DD-8, added during Stage 3's audit to catch a monitored file's outright deletion) works end to end, from a GitHub webhook to a posted PR comment and check run, verified against a real pull request on a live repository at v1.0.0. It's also been stress-tested against a 139-scenario adversarial benchmark, built specifically to find the tool's real limits -- see `redflag-ci-adversarial-tests/STRESS_TESTING.md` for what that testing found and `docs/adr/0001-deterministic-only-v1.md` for what the numbers below actually mean. v2.0.0's own additions since that live run -- cross-PR drift memory, the export functions, and Stage 3's fixes to existing detectors -- are validated by the automated test suite (unit tests against a mocked Octokit, plus integration tests exercising the full webhook-to-comment pipeline) and, for Stage 3 specifically, a dedicated 23-scenario adversarial stress-test sweep (`redflag-ci-adversarial-tests/STRESS_TEST_FINDINGS.md`), rather than that same live-repository run; `redflag-ci-adversarial-tests/STRESS_TESTING.md`'s new section explains why cross-PR behavior specifically is tested that way instead of via the benchmark corpus.
...
Full breakdown: `redflag-ci-adversarial-tests/benchmark/RESULTS.md`.
...
| [`redflag-ci-adversarial-tests/STRESS_TESTING.md`](redflag-ci-adversarial-tests/STRESS_TESTING.md) | How the benchmark grew from 18 to 139 scenarios, and what it found |
| [`redflag-ci-adversarial-tests/benchmark/RESULTS.md`](redflag-ci-adversarial-tests/benchmark/RESULTS.md) | The full benchmark corpus and results |
| [`redflag-ci-adversarial-tests/benchmark/COMPARISON.md`](redflag-ci-adversarial-tests/benchmark/COMPARISON.md) | A live comparison against Snyk Agent Scan (formerly mcp-scan) |
...
| [`redflag-ci-adversarial-tests/TRANSPARENCY_REPORT.md`](redflag-ci-adversarial-tests/TRANSPARENCY_REPORT.md) | The post-ship Stage 3 audit: dependency and dead-code checks, a full code-review pass, a coverage investigation, and a 23-scenario adversarial stress-test sweep |
| [`redflag-ci-adversarial-tests/STRESS_TEST_FINDINGS.md`](redflag-ci-adversarial-tests/STRESS_TEST_FINDINGS.md) | Stage 3's stress-test sweep, scenario by scenario, with real output |
