...
def analyze(files: List[DiffFile]) -> List[ScanFinding]:
    ...
    try:
                ...
                for pattern, severity, desc, rec in patterns:
                    if re.search(pattern, dl.content):
                        if pattern == r'\w+\[\w+\]\s*=\s*\w+':
                            if not any(x in dl.content for x in ['__proto__', 'constructor', 'prototype']):
                                if not re.search(r'req\.|params|query|body', dl.content):
                                    continue
        ...
        print(f'[DangerousPatternAnalyzer] error: {e}', file=sys.stderr)
    return findings
